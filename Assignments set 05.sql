...
SELECT productLine, REPLACE(productLine, 'Motorcycle', 'bicycle') AS updatedproductLine FROM products;

-- 10. REVERSE: Reverses the characters in a string.​
-- Q. 9. Reverse Customer Names
select * from customers;
SELECT customerName, REVERSE(customerName) AS reversedName FROM customers;

-- 11.LEFT: Returns the leftmost characters from a string.​
-- Q10. Extrcat first 4 letter from City
SELECT City, Left(City,4) AS ExtrctedChar FROM customers;

-- 12.RIGHT: Returns the rightmost characters from a string.​
-- Q11. Extrcat last 4 letter from City
SELECT City, right(City,4) AS ExtrctedChar FROM customers;
...
select * from products;
SELECT productCode, POSITION('S10' IN productCode) AS position FROM products ;
#OR we can filter "S10"
SELECT productCode, POSITION('S10' IN productCode) AS position FROM products WHERE productCode LIKE '%S10%';
...
select * from payments;
SELECT CONCAT('$', FORMAT(amount, 2)) AS formatted_amount FROM payments;
#with 0 decimal point
SELECT CONCAT('$', FORMAT(amount, 0)) AS formatted_amount FROM payments;
#e.g
